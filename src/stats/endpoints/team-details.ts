...
import type { TeamDetailsParams, TeamDetailsResponse } from '../types/team-details.js';
...
export async function teamDetails(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/teamdetails', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as TeamDetailsResponse;
}
