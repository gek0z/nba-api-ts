...
import type { TeamVsPlayerParams, TeamVsPlayerResponse } from '../types/team-vs-player.js';
...
export async function teamVsPlayer(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/teamvsplayer', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as TeamVsPlayerResponse;
}
