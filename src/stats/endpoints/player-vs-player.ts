...
import type { PlayerVsPlayerParams, PlayerVsPlayerResponse } from '../types/player-vs-player.js';
...
export async function playerVsPlayer(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/playervsplayer', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as PlayerVsPlayerResponse;
}
