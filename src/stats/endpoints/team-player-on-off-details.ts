...
import type { TeamPlayerOnOffDetailsParams, TeamPlayerOnOffDetailsResponse } from '../types/team-player-on-off-details.js';

const RESULT_SET_MAP: Record<string, string> = {
  'OverallTeamPlayerOnOffDetails': 'overallTeamPlayerOnOffDetails',
  'PlayersOffCourtTeamPlayerOnOffDetails': 'playersOffCourtTeamPlayerOnOffDetails',
...
export async function teamPlayerOnOffDetails(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/teamplayeronoffdetails', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as TeamPlayerOnOffDetailsResponse;
}
