...
import type { TeamPlayerDashboardParams, TeamPlayerDashboardResponse } from '../types/team-player-dashboard.js';

const RESULT_SET_MAP: Record<string, string> = {
  'PlayersSeasonTotals': 'playersSeasonTotals',
  'TeamOverall': 'teamOverall',
};

export async function teamPlayerDashboard(
  client: FetchClient,
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/teamplayerdashboard', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as TeamPlayerDashboardResponse;
}
