...
import type { VideoDetailsParams, VideoDetailsResponse } from '../types/video-details.js';
...
export async function videoDetails(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/videodetails', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as VideoDetailsResponse;
}
