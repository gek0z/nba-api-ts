...
import type { TeamGameLogsParams, TeamGameLogsResponse } from '../types/team-game-logs.js';
...
export async function teamGameLogs(
  ...
  const raw = await client.get<RawNBAStatsResponse>('/stats/teamgamelogs', apiParams);
  return parseStatsResponse(raw, RESULT_SET_MAP) as unknown as TeamGameLogsResponse;
}
